...
router.post('/register', function(req, res, next) {
  userModel.create(req.body.username, req.body.password, req.body.email, (err, user) => {
    if (err) {
      ...
      res.render('login', { title: 'Erreur', user: req.session.user,
                            errorMessage: errorMessage });
    } else {
      res.render('login', { title: 'Compte créé', user: req.session.user,
...
router.post('/login', function(req, res, next) {
  userModel.getByUsername(req.body.username, (err, user) => {
    ...
    res.render('login', { title: 'Connexion', user: req.session.user, successMessage: successMessage,
                          errorMessage: errorMessage });
  });
});
